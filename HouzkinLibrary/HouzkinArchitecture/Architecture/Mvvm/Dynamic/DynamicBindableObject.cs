...
namespace Houzkin.Architecture.Dynamic {
	...
	public class DynamicBindableObject<TModel> : DynamicBindableObject {
		/// <summary>新規インスタンスを初期化する。</summary>
		/// <param name="model">モデル</param>
		public DynamicBindableObject(TModel model) : base(model) { }
	...
	/// </summary>
	public class DynamicBindableObject : DynamicObject, INotifyPropertyChanged, IDisposable {

		/// <summary>新しいインスタンスを初期化する。</summary>
		/// <param name="model">参照するソース</param>
		public DynamicBindableObject(object model) {
			if (model == null) throw new ArgumentNullException(); //_model = null;
			else setModel(model);
		}
		/// <summary>MvpvmViewModelを初期化するときに呼び出す。</summary>
		internal DynamicBindableObject() { }
