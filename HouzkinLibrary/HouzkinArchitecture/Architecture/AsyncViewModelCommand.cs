...
namespace Houzkin.Architecture {
	...
	/// </summary>
	public class AsyncViewModelCommand : Command, ICommand, INotifyPropertyChanged {
		...
		public AsyncViewModelCommand(Action action, Func<bool> canExecute = null, Action<object> completed = null,
		Action<Exception> error = null ){
			...
			_worker.DoWork += (s, e) => {
				UIAction(new Action(() => this.RaiseCanExecuteChanged()));
				action();
			};
			_worker.RunWorkerCompleted += (s, e) => {

				if (_completed != null && e.Error == null)
					UIAction(new Action(()=>_completed(e.Result)));
				if (_error != null && e.Error != null)
					UIAction(new Action(() => _error(e.Error)));
				UIAction(new Action(() => this.RaiseCanExecuteChanged()));
			};
		}
