...
namespace Houzkin.Tree {
	...
	public class NodePath<T> : INodePath<T> {
		...
		public NodePath(params T[] path) : this(path ?? new T[0].AsEnumerable()){ }
		/// <summary>新規インスタンスを初期化する。</summary>
		...
		public static NodePath<T> Create<TNode>(TNode node, Converter<TNode, T> conv) where TNode : IReadOnlyTreeNode<TNode> {
	...
	public struct NodeIndex : INodePath<int>{
		...
		IEnumerator<int> IEnumerable<int>.GetEnumerator() {
			if (_nodePath == null) return new int[0].GetEnumerator() as IEnumerator<int>;
